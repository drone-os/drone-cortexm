...
use drone_stm32_drv_dma::dma::{Dma2Ch3Res, Dma2Ch4Res};
#[cfg(any(
  feature = "stm32l4r5",
  feature = "stm32l4r7",
  feature = "stm32l4r9",
...
macro_rules! spi_shared {
  (
    ...
    impl<$($tp: $bound,)*> SpiResCr1 for $name_res<$($tp,)* Frt> {
    ...
    impl<$($tp: $bound,)*> SpiResCr2 for $name_res<$($tp,)* Frt> {
...
macro_rules! spi {
  (
    ...
    macro_rules! $name_macro {
      ($reg:ident) => {
        <$crate::spi::Spi<_> as ::drone_core::drv::Driver>::new(
          $crate::spi::$name_res {
            $spi_cr1: $reg.$spi_cr1,
            $spi_cr2: $reg.$spi_cr2,
